...
print(f"[{strftime('%Y-%m-%d %H:%M:%S', gmtime())}] TCP ECHO Server is waiting for incoming connections on port {PORT} ... ")
        ...
        if sock == server_socket:
            ...
            try:
                ...
                if data:
                    ...
                    print(f"[{strftime('%Y-%m-%d %H:%M:%S', gmtime())}] Sending back to client {client_address} data: [{data.decode('utf-8')}] ... ")

            except:
                # print "[%s] Client (%s) is offline" % (strftime("%Y-%m-%d %H:%M:%S", gmtime()), client_address)
                print(f"[{strftime('%Y-%m-%d %H:%M:%S', gmtime())}] Client ({client_address}) is offline")
                sock.close()
